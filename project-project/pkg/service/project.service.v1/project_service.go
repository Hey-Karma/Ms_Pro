...
// 1.查询项目表
// 2.查询项目和成员的关联表 查到项目的拥有者 去member表查名字
// 3.查收藏表 判断收藏状态
func (ps *ProjectService) FindProjectDetail(ctx context.Context, msg *project.ProjectRpcMessage) (*project.ProjectDetailMessage, error) {
	...
	if err != nil {
		zap.L().Error("projct FindProjectDetail FindProjectByPIdAndMemId error", zap.Error(err))
		return nil, errs.GrpcError(model.DBError)
	}
	...
	if err != nil {
		zap.L().Error("projct rpc FindProjectDetail FindMemInfoById error", zap.Error(err))
		return nil, errs.GrpcError(model.DBError)
	}
	// 去member表查名字 去user模块找了
	// TODO: 优化 收藏的时候 可以放入redis
	isCollect, err := ps.projectRepo.FindCollectByPidAndMemId(c, projectCode, memberId)
	if err != nil {
		zap.L().Error("projct FindProjectDetail FindCollectByPidAndMemId error", zap.Error(err))
		return nil, errs.GrpcError(model.DBError)
	}
	if isCollect {
		projectAndMember.Collected = model.Collected
	}
	detailMsg := &project.ProjectDetailMessage{}
...
func (ps *ProjectService) UpdateDeletedProject(ctx context.Context, msg *project.ProjectRpcMessage) (*project.DeletedProjectResponse, error) {
	...
	if err != nil {
		zap.L().Error("projct RecycleProject DeleteProject error", zap.Error(err))
		return nil, errs.GrpcError(model.DBError)
	}
...
func (ps *ProjectService) UpdateProject(ctx context.Context, msg *project.UpdateProjectMessage) (*project.UpdateProjectResponse, error) {
	...
	if err != nil {
		zap.L().Error("projct UpdateProject::UpdateProject error", zap.Error(err))
		return nil, errs.GrpcError(model.DBError)
	}
...
func (ps *ProjectService) GetLogBySelfProject(ctx context.Context, msg *project.ProjectRpcMessage) (*project.ProjectLogResponse, error) {
	...
	if err != nil {
		zap.L().Error("projct ProjectService::GetLogBySelfProject::projectLogRepo.FindLogByMemberCode error", zap.Error(err))
		return nil, errs.GrpcError(model.DBError)
	}
	// 查询项目信息
	...
	if err != nil {
		zap.L().Error("projct ProjectService::GetLogBySelfProject:: projectRepo.FindProjectByIds error", zap.Error(err))
		return nil, errs.GrpcError(model.DBError)
	}
	...
	if err != nil {
		zap.L().Error("projct ProjectService::GetLogBySelfProject:: taskRepo.FindTaskByIds error", zap.Error(err))
		return nil, errs.GrpcError(model.DBError)
	}
